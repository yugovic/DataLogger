...
### Phase 0a: データ品質基盤（最優先）
1. ✅ Firebase認証実装
2. ✅ セットアップデータの保存・読み込み機能
3. ⬜ デモ用初期値の廃止・null保存（偽データ混入の根絶）
4. ⬜ スキーマ検証（zod）・入力バリデーション・単位表示
5. ⬜ 品質ゲート（lint/typecheck/CI）・計測イベント導入

### Phase 0b: 記録・振り返り体験
6. ⬜ セットアップ比較ビュー（前回vs今回の差分ハイライト）
7. ⬜ 前回値コピー機能（入力負荷削減）
8. ⬜ 履歴・検索強化、モバイル最適化

### Phase 0c: 共有（Give-to-Get）
9. ⬜ セットアップ公開設定（visibility）とFirestoreルール拡張
10. ⬜ 共有ユーザー間の相互閲覧・匿名化オプション

### Phase 1: マーケットプレイス
11. ⬜ ロガー連携（AIM CSV / デジスパイスNMEAパーサー）
12. ⬜ Stripe Connect決済・出品/購入フロー
13. ⬜ レビュー・売り手実績表示

### Phase 2: 集合知・サブスク
14. ⬜ ベンチマーク統計（車種×サーキット）・条件マッチング
15. ⬜ テレメトリ高度分析
