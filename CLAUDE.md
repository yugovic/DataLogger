...
- Firebase連携の統合テスト
- E2Eテストでユーザーフローを確認
- **実装後は `docs/testing/regression-checklist.md` の該当セクションを実行する**（自動テスト＋変更範囲別の手動確認項目一覧。機能追加時はここに追記して更新すること）
- **実装後は `docs/feature-ledger.md`（機能マザー台帳）も該当行を追加・更新する**（機能の追加・変更・削除、状態（✅/⚠️/🚧/🗑️）の変化を反映。レビュー時もこの台帳と突き合わせる）
